...
You can from left to right: • auto-code exact text • auto-code sentences based on a text fragment in the current file • auto-code sentences based on a text fragment for all files • auto-code the current file using start and end text marks. You can use ‘\\n’ for a line ending character
...
After walking the basics with Sarah she felt confident exploring the data herself. She spend a few weeks focusing on Q4 and came back with some initial analysis so we can walk her through the reporting features in QualCoder.

So before we move forward, please download the Influencers-final.qda **\[fixme: add to the folder\]** project and open it.
