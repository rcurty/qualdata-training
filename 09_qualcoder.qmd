...
Do not close the terminal or command prompt running in the background; otherwise, you will exit QualCoder. If you miminize the QualCoder window and get lost, you may click the Python Launch icon.
:::
...
We've created a project, but we don't have any files yet, so let's make sure to load them now. Choose, manage files, and then, the second icon as noted below (Import File Into Project Folder), select the desired folder and files.

![](images/add-files-02.png)

Note the file ID will be assigned according to the order of the import, so if you want that to match your file naming convention, you should order it accordingly. In case you need to fix it, click the red X on the right and delete the files, then reorder then accordingly and bring import them again into QualCoder.

You may double click in the file or choose the "eye icon" to open the file content. Don't worry about the information at the bottom of the window for now, we will cover that later.
...
Now, let's link the corresponding file to the specific case we have created. Click on the file cell to manage files for the case. This will prompt another window as shown below:
...
Each transcript has some characteristics associated with interviewees. It would be helpful to describe these as attributes for further analysis. We will only add two types of attributes: `age group` and `niche`. There are two different ways to create attributes: via `manage > manage attributes` or in the `manage file` window you may select the `blue plus sign`. In this case, the latter might be just more convenient considering we have only a few attributes and cases.
...
For now, we won't need to add memos, but keep in mind that they can be useful for providing additional context when attributes aren’t immediately clear.

We now have a well-organized project set up in QualCoder, so let’s jump into some coding!

------------------------------------------------------------------------

**Recommended/Cited Sources:**

Cheung, K. K. C., & Tai, K. W. H. (2021). The use of intercoder reliability in qualitative interview data analysis in science education. *Research in Science & Technological Education*, *41*(3), 1155–1175. https://doi.org/10.1080/02635143.2021.1993179Cheung, K. K. C., & Tai, K. W. H. (2021). The use of intercoder reliability in qualitative interview data analysis in science education. *Research in Science & Technological Education*, *41*(3), 1155–1175. https://doi.org/10.1080/02635143.2021.1993179
